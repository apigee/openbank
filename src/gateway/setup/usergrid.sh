...
ADMIN_EMAIL=seshireddy@apigee.com
APW=ses2@hiRR
...
resp=`curl -X POST ${URI}/${ORG}/${APP_NAME}/banks?access_token="${token}"`
echo "Status: Creating Banks Collection: ${resp}"

resp=`curl -X POST ${URI}/${ORG}/${APP_NAME}/accounts?access_token="${token}"`
echo "Status: Creating Accounts Collection:${resp}"

resp=`curl -X POST ${URI}/${ORG}/${APP_NAME}/customers?access_token="${token}"`
echo "Status: Creating Customers Collection:${resp}"

resp=`curl -X POST ${URI}/${ORG}/${APP_NAME}/transactions?access_token="${token}"`
