...
var log = bunyan.createLogger({name: 'cleanup_locations'});
...
function getToken() {
    return new Promise(function (resolve, reject) {
        log.info('Getting access token from BaaS');
...
function getLocations(token, cursor, locations) {
    return new Promise(function (resolve, reject) {
        log.info('getting locations with cursor ' + cursor);
        ...
        request(options, function (err, resp, body) {
            ...
            if (body.cursor) {
                log.info('cursor obtained, recursing...');
                ...
                log.info('no more cursor; resolving...');
                resolve({
                    token: token,
                    locations: locations
                });
...
function updateLocation(token, location) {
    return new Promise(function (resolve, reject) {
        log.info('saving locations ' + location.name);
